...
function displayTag( $tag, $param ) {
	...
	if ( ! isset( $param["selectedTag"] ) ) $param["selectedTag"] = false;
	...
	if ($param["selectedTag"] == $tag->name) {
		if ($tag->name && substr($tag->name, 0, 1) != ":") {
			...
			if (!empty($subtags)) $class[] = "has-subtags";
			if (!isset($param["selectedSubTag"])) $class[] = "active";
		} else {
			$class[] = "active";
		}
	}
	...
	if (!empty($class)) {
		?> class="<?php echo implode(" ", $class); ?>"<?php
	}
	if ($tag->name === false) {
		?> data-all="true"><?php
	} else {
		?> data-tag="<?php echo $tag->name; ?>"<?php
		global $user;
		if (false&&$special_tag = $user->specialTag($tag->name)) {
			foreach ( $special_tag as $t ) {
				if (!isset($t["type"])) continue;
				if ($t["type"] == "color") echo ' data-color="', $t["options"]["hex"], '" style="border-left: 2px solid ', $t["options"]["hex"], '"';
				elseif ($t["type"] == "todo") echo ' data-todo="true"';
			}
		}
		?>><?php
	}
	?><a href="/thinkery/<?php echo $tag->slug; ?>" title="<?php printf( _n( '%s thing', '%s things', $tag->count, 'thinkery' ), $tag->count ); ?>"><?php
		...
		?> <small class="num"><?php echo $tag->count; ?></small><?php
		if ($tag->name !== false && substr($tag->name, 0, 1) != ":") {
	...
	if (!empty($subtags)) {
		...
		foreach ( $subtags as $subtag => $count ) {
			if ($c == 0 && $count < $min_count) $min_count = 1;
			if ($count < $min_count || $c++ > 4) {
				if ($subtag_found) break;
				if ($subtag != $selectedSubTag) continue;
			}
			?><li data-tag="<?php echo $tag->name; ?>" data-subtag="<?php echo $subtag; ?>"<?php
			if ($subtag === $selectedSubTag) {
				echo ' class="active"';
				$subtag_found = true;
			}
			?>><?php
			?><a href="/thinkery/<?php echo $tag->slug; ?>/<?php echo $subtag->slug; ?>" title="<?php printf( _n( '%s thing', '%s things', $tag->count, 'thinkery' ), $tag->count ); ?>"><?php echo $subtag; ?> <small class="num"><?php echo $subtag->count; ?></small></a></li><?php
		}
		?></ul><?php
	}
	?></li><?php
...
<ul class="tags" id="nonFavoriteTags">
	...
	<?php if ($display_user->tags->lastTotal > $limit): ?>
		<div class="moretags<?php if (isset($_GET["full"])) echo " loaded"; ?>">
	...
	<nav id="filter">
		<ul>
				<li>
					<a class="button action-show-archived" href="<?php if (!isset($_GET["archived"])) echo '?archived'; else {
						if (isset($_GET["tag"])) {
